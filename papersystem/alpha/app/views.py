...
def show_tec_forms():
	if 'username' in session:
		userid = session['username']
		username = forms.show_tecname_from_db(userid)
	else:
		statecode=-2 #返回请登录页面
	Loginform = LoginForm()
	tag = request.form.get('tag')
	if tag == 'academic_paper':
		...
		Monoform = MonoForm()
		if request.method == 'POST':
			forms.add_info_to_db(Monoform.data)
	elif tag == 'patent':
		Patform = PatForm()
		if request.method == 'POST':
			forms.add_info_to_db(Patform.data)
	elif tag == 'project':
		Proform = ProForm()
		if request.method == 'POST':
			forms.add_info_to_db(Proform.data)
...
@app.route('/xlsdownload01', methods=['GET','POST'])
def xlsdownload00():
	Postid=[]
	resultset=[]
	datax = request.form.to_dict()  
...
def xlsdownload04():
	headerList = [u"论文名称", u"发表年份", u"第一作者"]
	dataList = [
		(u"李磊", u"男", u"20"),
		(u"王艳", u"女", u"18"),
	]
