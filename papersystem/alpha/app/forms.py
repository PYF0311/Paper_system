...
def verify_tec_exist(user):
    ...
    try :
        with db.cursor() as cursor:
            sql = "SELECT tecId FROM tec_info where tecId = %s"
            cursor.execute(sql,(user,))
            result = str(cursor.fetchone())
            result = re.sub('[^a-zA-Z0-9]',"",result)
            #print("result:",result)
...
def verify_tec_password(username,password):
    ...
    try:
        with db.cursor() as cursor:
            sql = "SELECT password FROM tec_info WHERE tecId = %s"
            cursor.execute(sql,(username,))
            result = str(cursor.fetchone())
            result = re.sub('[^a-zA-Z0-9]',"",result)
            if password == result:
    ...
    if tag == 'academic_paper':
        ...
        try:
            with db.cursor() as cursor:
                ...
                sql1 = "select count(*) from publication_info;"
                cursor.execute(sql1)
                result=str(cursor.fetchone())
                querynum=int(re.sub('["(),\']','',result))+1
        ...
        try:
            with db.cursor() as cursor:
                sql1 = "select count(*) from standard;"
                cursor.execute(sql1)
                result = str(cursor.fetchone())
                querynum = int(re.sub('["(),\']','',result))+1
        ...
        try:
            with db.cursor() as cursor:
                sql1 = "select count(*) from degree_paper;"
                cursor.execute(sql1)
                result = str(cursor.fetchone())
                querynum = int(re.sub('["(),\']','',result))+1
