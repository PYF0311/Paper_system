...
def show_aca_info():
	...
	print(resultset)
	return render_template("/stu/aca_info.html",resultset = resultset)

@app.route('/stu/deg_info',methods=['GET','POST'])
def show_deg_info():
	resultset = session['resultset']
	return render_template("/stu/deg_info.html",resultset = resultset)

@app.route('/stu/pat_info',methods=['GET','POST'])
def show_pat_info():
	resultset = session['resultset']
	print(resultset)
	return render_template("/stu/pat_info.html",resultset = resultset)

@app.route('/stu/pro_info',methods=['GET','POST'])
def show_pro_info():
	resultset = session['resultset']
	return render_template("/stu/pro_info.html",resultset = resultset)

@app.route('/stu/mono_info',methods=['GET','POST'])
def show_mono_info():
	resultset = session['resultset']
	return render_template("/stu/mono_info.html",resultset = resultset)

@app.route('/stu/stand_info',methods=['GET','POST'])
def show_stand_info():
	resultset = session['resultset']
	print(resultset)
	return render_template("/stu/stand_info.html",resultset = resultset)

@app.route('/stu/aca_edit',methods=['GET','POST'])
def show_aca_edit():
	resultset = session['resultset']
	print(resultset)
	return render_template("/stu/aca_edit.html",resultset = resultset)

@app.route('/stu/deg_edit',methods=['GET','POST'])
def show_deg_edit():
	resultset = session['resultset']
	print(resultset)
	return render_template("/stu/deg_edit.html",resultset = resultset)

@app.route('/stu/pat_edit',methods=['GET','POST'])
def show_pat_edit():
	resultset = session['resultset']
	print(resultset)
	return render_template("/stu/pat_edit.html",resultset = resultset)

@app.route('/stu/pro_edit',methods=['GET','POST'])
def show_pro_edit():
	resultset = session['resultset']
	print(resultset)
	return render_template("/stu/pro_edit.html",resultset = resultset)

@app.route('/stu/mono_edit',methods=['GET','POST'])
def show_mono_edit():
	resultset = session['resultset']
	print(resultset)
	return render_template("/stu/mono_edit.html",resultset = resultset)

@app.route('/stu/stand_edit',methods=['GET','POST'])
def show_stand_edit():
	resultset = session['resultset']
	print(resultset)
	return render_template("/stu/stand_edit.html",resultset = resultset)
...
def show_academic_manage_tec():
	...
	length = len(results)
	return render_template("/tec/academic.html",results = results,length = length)
...
def show_standard_manage_tec():
	...
	length = len(results)
	print(results)
	return render_template("/tec/standard.html",results = results,length = length)	
...
def show_login():
	...
	if request.method == 'POST':
		...
		if user is True and forms.verify_admin_password(form.username.data,form.password.data):
			session['username']=username
			return redirect(url_for('show_academic_manage'))
		else:
			statecode = -2 #返回请登录界

	return render_template("/su/index.html", form=form)
...
@app.route("/su/aca_upload",methods=['GET','POST'])
def upload_file():
	if request.method =='POST':
...
def load_file(filename):
	...
	try:
		...
		edit = df.iloc[:,2:10]
		engine = create_engine(str("mysql+pymysql://"+'%s' + ':%s' + "@%s/%s") % ('root', 'JMU-bec2017', 'localhost', 'PaperSystem'))

		edit.to_sql('academic_paper_edit', con=forms.connect_db(), if_exists='append', index=False)
