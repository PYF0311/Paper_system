jQuery(document).ready(function(){
    ...
    academic.filed = [
        ...
        {"data": "acadPaperId"},
        {"data": "acadPaperNameCH"},
        {"data": "acadPaperNameEN"},
        {"data": "firstEdit"},
        {"data": "secondEdit"},
        {"data": "thirdEdit"},
        {"data": "transEdit"},
	...
	jQuery('.detailbutton').click(function(){
	var allarr=getdata();
	if(allarr.length){
		 $.ajax({
                   url: "/stu/info", 
	...
	jQuery('.addbutton').click(function(){
		window.open("/tec/forms");					//alert to 请选择数据
	});
	...
	function getdata(){
		...
		if(uuids.length){
			for(var i=0;i<uuids.length;i++){
				allarr.push(setObject(uuids[i].acadPaperId,uuids[i].type));
			}
		}
		return allarr;
	};
