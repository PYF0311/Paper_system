jQuery(document).ready(function(){
    ...
    employee.status = [
       ...
       {"targets": [1], "visible": false},    //设置第1 列隐藏/显示
	...
	function checkIndex(){
		...
		function getdata(){
			...
			if(uuids.length){
				...
				allarr.push(setObject(uuids[i].tecId));
	            }
	        }
	        return allarr;
	    };
	...
	//教师添加
	    $("#tec_add_form button").click(function(){ 
	       $.ajax({
	         url:"",  //教师添加链接
	         data:$("#tec_add_form").serialize(), //表单数据序列化, 可以对form表单进行序列化，从而将form表单中的所有参数传递到服务端。
	         dataType: "json",
	         type:"POST",
	         success:function(data){
                alert(data.msg);
